...
<record id="my_record" model="my.model">
    ...
    <field name="datetime" eval="datetime.now()"/>
...
**Result**: MM00001, MM00002, MM00003, ...

### 4.2 Use Sequence in Model
...
<odoo>
    <data noupdate="1">
        ...
        <record id="category_computers" model="product.category">
            <field name="name">Computers</field>
            <field name="parent_id" ref="category_electronics"/>
        </record>
    </data>
</odoo>
```
...
<record id="action_server_create_task" model="ir.actions.server">
    ...
    <field name="fields_lines" eval="[
        (0, 0, {'col1': 'name', 'value': 'object.name'}),
        (0, 0, {'col1': 'partner_id', 'value': 'object.partner_id.id'}),
    ]"/>
</record>
```
...
```xml
<!-- Use noupdate="1" for production data -->
<data noupdate="1">
    <record id="default_category" model="product.category">
...
```xml
<!-- Don't use noupdate="0" for production data -->
<data noupdate="0">
    <record id="important_config" model="res.config">
        <!-- Will be overwritten on upgrade! -->
    </record>
</data>

<!-- Don't hardcode IDs -->
<field name="partner_id" eval="1"/>  <!-- ❌ Use ref() -->

<!-- Don't forget noupdate flag -->
<odoo>
